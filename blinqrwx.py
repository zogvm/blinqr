...
def decoded_img(lock,decode_i,index,q_img,q_decode):
    ...
    while True:
        ...
        if q_img.qsize()>0:	
            ...
            img=q_img.get()
            decoded_qrs,pts = qrcode_detector.detectAndDecode(img)
...
def read_decode(index,q_decode,q_block):
    ...
    while True:
        ...
        if q_decode.qsize()>0:	
            ...
            data_decoded = b85decode(decoded_qr)
...
#接收视频
def receive(path):
    print(colored(f'多进程微信模式 useRGB:{useRGB}', 'blue'))
    global isEnd       
    ...
    #进程队列
    q_img = multiprocessing.Queue(10)
    q_decode = multiprocessing.Queue(10)
    q_block = multiprocessing.Queue(10)
